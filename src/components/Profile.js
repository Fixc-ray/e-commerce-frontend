...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    if (!userData || !token) {
      ...
      axios.get(`${url}/api/user/products`, {
  ...
  const handleDeleteProduct = (productId) => {
    const token = localStorage.getItem('token');
    axios.delete(`https://e-commerce-backend-uvz8.onrender.com/api/products/${productId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    .then(() => {
  ...
  return (
    <div className="mt-20 p-10 bg-gray-100 flex items-center justify-center">
